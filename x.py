...
class SpaceStudioIntro(QWidget):
    ...
    def _on_finished(self) -> None:
        if hasattr(self, "finished_callback") and self.finished_callback:
            QTimer.singleShot(150, self.finished_callback)
    ...
    def _draw_brand(
        ...
        try:
            ...
            version_font = QFont("Segoe UI", max(10, int(x_size * 0.18)))
            version_font.setLetterSpacing(QFont.AbsoluteSpacing, 1.5)
            painter.setFont(version_font)
            painter.setPen(QColor(120, 120, 125, int(alpha * 0.8)))
            v_metrics = QFontMetrics(version_font)
            v_text = f"Version {APP_VERSION}"
            v_width = v_metrics.horizontalAdvance(v_text)
            # Position at the very bottom of the screen
            v_y = (cy * 2) - 40
            painter.drawText(int(cx - v_width / 2), int(v_y), v_text)
        except ImportError:
            pass
